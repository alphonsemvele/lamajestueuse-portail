...
export default function PostForm({ post }: { post: Post | null }) {
    ...
    const submit = (event: FormEvent) => {
        ...
        if (editing) {
            router.post(routes.admin.post(post.slug), { ...data, _method: 'put' }, options);
        } else {
            submitPost(routes.admin.posts, options);
        }
    };

    return (
        <AdminLayout
            title={editing ? t('Modifier la publication') : t('Nouvelle publication')}
            heading={editing ? post.title : t('Nouvelle publication')}
            subheading={t('Alimente le centre d’information du tableau de bord.')}
        >
            <form onSubmit={submit} className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                <Card className="space-y-5 p-6">
                    <div>
                        <Label htmlFor="title" required>{t('Titre')}</Label>
                ...
                <div className="space-y-4 xl:sticky xl:top-24 xl:self-start">
                    <Card className="space-y-4 p-5">
                        <div>
                            <Label htmlFor="type" required>{t('Rubrique')}</Label>
